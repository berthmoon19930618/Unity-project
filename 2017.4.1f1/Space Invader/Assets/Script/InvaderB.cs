...
public class InvaderB : MonoBehaviour {
    public GameObject ExplodeB;
    ...
    {
        if (collision.tag=="BulletB"|| collision.tag == "Ship")
        {
            Destroy(collision.gameObject);
            Destroy(this.gameObject);
            Instantiate(ExplodeB, this.gameObject.transform.position, Quaternion.identity);
            if (collision.tag == "Ship")//如果打到太空船,太空船也播爆炸動畫
            {
                Instantiate(ExplodeB, collision.gameObject.transform.position, Quaternion.identity);
                GameFunction.Instance.GameOver();
            }
        }
        if (collision.tag == "BulletB")
        {
